...
public class RangeQueries {
    public static void main(String args[]) throws IOException {
        ...
        BufferedWriter bw = new BufferedWriter(new FileWriter(metricsPath+ File.separator+"range-queries-trajparquet-"+ Paths.get(queriesFilePath).getFileName().toString().replaceFirst("\\.[^.]+$", "")+"-"+Paths.get(parquetPath).getFileName().toString()+".txt"));
...
//                        comparator = comparator.thenComparingDouble(d-> d.getSpatioTemporalPoints()[0].getLongitude());
        ...
        bw = new BufferedWriter(new FileWriter(metricsPath+ File.separator+"metrics-range-queries-trajparquet-"+ Paths.get(queriesFilePath).getFileName().toString().replaceFirst("\\.[^.]+$", "")+"-"+Paths.get(parquetPath).getFileName().toString()+".txt"));
