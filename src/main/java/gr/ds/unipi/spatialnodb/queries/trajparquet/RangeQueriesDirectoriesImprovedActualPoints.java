...
public class RangeQueriesDirectoriesImprovedActualPoints {
    public static void main(String args[]) throws IOException {
        ...
        BufferedWriter bw = new BufferedWriter(new FileWriter(metricsPath+ File.separator+"range-queries-directoriesImprovedActualPoints-"+Paths.get(parquetPath).getFileName().toString()+"-"+ Paths.get(queriesFilePath).getFileName().toString().replaceFirst("\\.[^.]+$", "")+".txt"));
                ...
                if(indexUtils instanceof IndexUtils2D) {
                    fullyIntersectedPairRDD = fullyIntersectedPairRDD.flatMapValues(f->{
                        ...
                        for (SpatioTemporalPoint spatioTemporalPoint : spatioTemporalPoints) {
                            if (HilbertUtil.inTimeWindow(spatioTemporalPoint.getTimestamp(), queryMinTimestamp, queryMaxTimestamp)) {
                                currentSpatioTemporalPoints.add(spatioTemporalPoint);
                            }else{
                                if(!currentSpatioTemporalPoints.isEmpty()){
                                    break;
                                }
...
//        }
//        bw = new BufferedWriter(new FileWriter(metricsPath+ File.separator+"metrics-range-queries-directoriesImprovedActualPoints-"+Paths.get(parquetPath).getFileName().toString()+"-"+ Paths.get(queriesFilePath).getFileName().toString().replaceFirst("\\.[^.]+$", "")+".txt"));
...
//        bw.close();
        ...
        if(sparkConf.getBoolean("spark.eventLog.enabled",false)){
            ...
            try {
                SparkLogParser.enrichQueryAdHocFile(metricsPath+ File.separator+"range-queries-directoriesImprovedActualPoints-"+Paths.get(parquetPath).getFileName().toString()+"-"+ Paths.get(queriesFilePath).getFileName().toString().replaceFirst("\\.[^.]+$", "")+".txt", lists);
            }catch (Exception e) {
                e.printStackTrace();
            }
            eventLogFile.delete();
