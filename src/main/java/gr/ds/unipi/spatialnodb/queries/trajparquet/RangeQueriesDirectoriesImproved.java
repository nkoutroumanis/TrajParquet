...
public class RangeQueriesDirectoriesImproved {
    public static void main(String args[]) throws IOException {
        ...
        BufferedWriter bw = new BufferedWriter(new FileWriter(metricsPath+ File.separator+"range-queries-directoriesImproved-"+Paths.get(parquetPath).getFileName().toString()+"-"+ Paths.get(queriesFilePath).getFileName().toString().replaceFirst("\\.[^.]+$", "")+".txt"));
...
//        }
//        bw = new BufferedWriter(new FileWriter(metricsPath+ File.separator+"metrics-range-queries-directoriesImproved-"+Paths.get(parquetPath).getFileName().toString()+"-"+ Paths.get(queriesFilePath).getFileName().toString().replaceFirst("\\.[^.]+$", "")+".txt"));
...
//        bw.close();
        ...
        if(sparkConf.getBoolean("spark.eventLog.enabled",false)){
            ...
            try {
                SparkLogParser.enrichQueryAdHocFile(metricsPath + File.separator + "range-queries-directoriesImproved-" + Paths.get(parquetPath).getFileName().toString() + "-" + Paths.get(queriesFilePath).getFileName().toString().replaceFirst("\\.[^.]+$", "") + ".txt", lists);
            }catch (Exception e) {
                e.printStackTrace();
            }
            eventLogFile.delete();
