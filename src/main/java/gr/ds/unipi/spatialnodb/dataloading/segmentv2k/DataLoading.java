package gr.ds.unipi.spatialnodb.dataloading.segmentv2k;
...
import gr.ds.unipi.spatialnodb.messages.common.segmentv2.TrajectorySegmentWriteSupport;
...
public class DataLoading {
    public static void main(String args[]) throws IOException {
        ...
        JavaPairRDD rdd = jsc.textFile(rawDataPath).map(f->f.split(delimiter)).groupBy(f-> f[objectIdIndex]).flatMapToPair(f-> {
            ...
            SpatioTemporalPoints.Builder currentPart = SpatioTemporalPoints.newBuilder();
            ...
            for (int i = 0; i < tuple.size(); i++) {
                ...
                currentPart.addSpt(SpatioTemporalPoints.SpatioTemporalPoint.newBuilder().setLongitude(tuple.get(i)._1()).setLatitude(tuple.get(i)._2()).setTimestamp(tuple.get(i)._3()).build());

                if(currentPart.getSptCount()==segK){
                    ...
                    for (int j = 0; j < currentPart.getSptCount(); j++) {
                        if (Double.compare(minLongitude, currentPart.getSpt(j).getLongitude()) == 1) {
                            minLongitude = currentPart.getSpt(j).getLongitude();
                        }
                        if (Double.compare(minLatitude, currentPart.getSpt(j).getLatitude()) == 1) {
                            minLatitude = currentPart.getSpt(j).getLatitude();
                        }
                        if (Long.compare(minTimestamp, currentPart.getSpt(j).getTimestamp()) == 1) {
                            minTimestamp = currentPart.getSpt(j).getTimestamp();
                        }
                        if (Double.compare(maxLongitude, currentPart.getSpt(j).getLongitude()) == -1) {
                            maxLongitude = currentPart.getSpt(j).getLongitude();
                        }
                        if (Double.compare(maxLatitude, currentPart.getSpt(j).getLatitude()) == -1) {
                            maxLatitude = currentPart.getSpt(j).getLatitude();
                        }
                        if (Long.compare(maxTimestamp, currentPart.getSpt(j).getTimestamp()) == -1) {
                            maxTimestamp = currentPart.getSpt(j).getTimestamp();
                        }
                    }

                    long[] hil = HilbertUtil.scaleGeoTemporalPoint(currentPart.getSpt(0).getLongitude(), minLon, maxLon, currentPart.getSpt(0).getLatitude(), minLat, maxLat, currentPart.getSpt(0).getTimestamp(), minTime, maxTime, maxOrdinates);
                    Ranges ranges = ((SmallHilbertCurve)smallHilbertCurveBr.getValue()).query(hil, hil, 0);
                    long hilbertValue = ranges.toList().get(0).low();

                    trajectoryParts.add(Tuple2.apply(hilbertValue,new TrajectorySegment(objectId, part++, currentPart.build(), minLongitude, minLatitude, minTimestamp, maxLongitude, maxLatitude, maxTimestamp)));
