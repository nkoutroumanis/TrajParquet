package gr.ds.unipi.spatialnodb.dataloading.geoparquetv2;
...
import gr.ds.unipi.spatialnodb.messages.common.geoparquetv2.TrajectoryWriteSupport;
...
//        ParquetOutputFormat.setPageSize(job, (1048576/10));
...
//                System.out.println(objectId+" "+tuple.get(i)._1() +" "+ minLon+" "+ maxLon+" "+ tuple.get(i)._2()+" "+ minLat+" "+ maxLat+" "+ tuple.get(i)._3()+" "+ minTime+" "+ maxTime+" "+ maxOrdinates);
                ...
                if(currentHilValue != hilbertValue){
                    ...
                    for (int k = 0; k < currentPart.size(); k++) {
                        coordinates[k] = new Coordinate(currentPart.get(k)._1(),currentPart.get(k)._2(),currentPart.get(k)._3());
                    }

                    trajectoryParts.add(Tuple2.apply(currentHilValue,new Trajectory(objectId, part++, g.createLineString(coordinates), minLongitude, minLatitude, minTimestamp, maxLongitude, maxLatitude, maxTimestamp)));
