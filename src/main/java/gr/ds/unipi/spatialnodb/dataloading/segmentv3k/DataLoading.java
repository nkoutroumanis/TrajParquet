package gr.ds.unipi.spatialnodb.dataloading.segmentv3k;
        ...
        JavaPairRDD rdd = jsc.textFile(rawDataPath).map(f->f.split(delimiter)).groupBy(f-> f[objectIdIndex]).flatMapToPair(f-> {
                    ...
                    for (int i = 0; i < tuple.size(); i++) {
                        ...
                        if(currentPart.size()==segK) {
                            ...
                            trajectoryParts.add(Tuple2.apply(hilbertValue,new Trajectory(objectId, part++, BsonDocumentWrapper.parse(new LineString(coordinates).toJson())/*Document.parse(new LineString(coordinates).toJson()).toBsonDocument()*/, timestamps, minLongitude, minLatitude, minTimestamp, maxLongitude, maxLatitude, maxTimestamp)));
