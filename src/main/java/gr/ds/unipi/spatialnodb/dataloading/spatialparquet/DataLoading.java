package gr.ds.unipi.spatialnodb.dataloading.spatialparquet;
...
import gr.ds.unipi.spatialnodb.messages.common.spatialparquet.TrajectorySegmentWriteSupport;
...
//        final long maxTime = hilbert.getLong("maxTime");
        ...
        sparkConf.setAppName("Trajectory Loading in SpatialParquet");
        ...
        try(BufferedWriter bf = new BufferedWriter(new FileWriter(metricsPathExport+ File.separator+"data-loading-spatialparquet-"+ Paths.get(writePath).getFileName().toString()+".txt"))) {
            bf.write("Write Time");
            bf.newLine();
