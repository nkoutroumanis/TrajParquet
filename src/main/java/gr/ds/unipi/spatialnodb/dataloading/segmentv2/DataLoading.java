package gr.ds.unipi.spatialnodb.dataloading.segmentv2;
...
public class DataLoading {
    public static void main(String args[]) throws IOException {
        ...
        JavaPairRDD rdd = jsc.textFile(rawDataPath).map(f->f.split(delimiter)).groupBy(f-> f[objectIdIndex]).flatMapToPair(f-> {
            ...
            currentPart.addSpt(SpatioTemporalPoints.SpatioTemporalPoint.newBuilder().setLongitude(tuple.get(0)._1()).setLatitude(tuple.get(0)._2()).setTimestamp(tuple.get(0)._3()).build());
...
//                System.out.println(objectId+" "+tuple.get(i)._1() +" "+ minLon+" "+ maxLon+" "+ tuple.get(i)._2()+" "+ minLat+" "+ maxLat+" "+ tuple.get(i)._3()+" "+ minTime+" "+ maxTime+" "+ maxOrdinates);
                ...
                if(currentHilValue != hilbertValue){
                    ...
                    trajectoryParts.add(Tuple2.apply(currentHilValue,new TrajectorySegment(objectId, part++, currentPart.build(), minLongitude, minLatitude, minTimestamp, maxLongitude, maxLatitude, maxTimestamp)));
