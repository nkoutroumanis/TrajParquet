package gr.ds.unipi.spatialnodb.dataloading.segmentv3;
...
import gr.ds.unipi.spatialnodb.messages.common.segmentv3.TrajectoryWriteSupport;
...
public class DataLoading {
    ...
    public static void main(String args[]) throws IOException {
        ...
        SparkConf sparkConf = new SparkConf()/*.setMaster("local[1]").set("spark.executor.memory","1g")*/.registerKryoClasses(new Class[]{SmallHilbertCurve.class, HilbertUtil.class, Encoder.class, BsonDocumentCodec.class});
...
//                System.out.println(objectId+" "+tuple.get(i)._1() +" "+ minLon+" "+ maxLon+" "+ tuple.get(i)._2()+" "+ minLat+" "+ maxLat+" "+ tuple.get(i)._3()+" "+ minTime+" "+ maxTime+" "+ maxOrdinates);
                ...
                if(currentHilValue != hilbertValue){
                    ...
                    List<Position> coordinates = new ArrayList<>(currentPart.size());
                    long[] timestamps = new long[currentPart.size()];

                    for (int k = 0; k < currentPart.size(); k++) {
                        coordinates.add(new Position(currentPart.get(k)._1(),currentPart.get(k)._2()));
                        timestamps[k] = currentPart.get(k)._3();
                    }

                    trajectoryParts.add(Tuple2.apply(currentHilValue,new Trajectory(objectId, part++, BsonDocumentWrapper.parse(new LineString(coordinates).toJson())/*Document.parse(new LineString(coordinates).toJson()).toBsonDocument()*/, timestamps, minLongitude, minLatitude, minTimestamp, maxLongitude, maxLatitude, maxTimestamp)));
