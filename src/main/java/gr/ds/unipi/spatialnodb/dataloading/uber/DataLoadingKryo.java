...
public class DataLoadingKryo {
    ...
    public DataLoadingKryo(String rawDataPath, String writePath, int longitudeIndex, int latitudeIndex, int idIndex, String delimiter, int resolution) {
    ...
    public DataLoadingKryo(String rawDataPath, String writePath, int longitudeIndex, int latitudeIndex, int idIndex, String delimiter, double radius, double minLon, double minLat, double maxLon, double maxLat) {
        this.rawDataPath = rawDataPath;
        this.writePath = writePath;
        this.longitudeIndex = longitudeIndex;
        this.latitudeIndex = latitudeIndex;
    ...
    public void startLoading() throws IOException {
        ...
        UserDefinedFunction serializationKryo = null;
...
//            H3Core h3 = H3Core.newInstance();
            ...
            Kryo kr = new Kryo();
            //kr.register(gr.ds.unipi.spatialnodb.shapes.Point.class);
            //Broadcast<Kryo> kryoBr = jsc.broadcast(kr);
            ...
            serializationKryo = udf(
                    (String x, String y) -> new gr.ds.unipi.spatialnodb.shapes.Point(Double.parseDouble(x), Double.parseDouble(y)).toByteArray(kr), DataTypes.BinaryType
            );
        ...
        df = df.withColumn("PointBinary", serializationKryo.apply(col("longitude"),col("latitude"))).withColumn("index", index.apply(col("longitude"),col("latitude"))).withColumn("longitude",col("longitude").cast(DOUBLE.toString())).withColumn("latitude",col("latitude").cast(DOUBLE.toString())).repartition(col("index"));
